#!/usr/bin/env python3
"""
計算暴跌清單。跟爆量掃描(compute_scan.py)完全對稱,共用同一個 vol_ratio
門檻,差別只在價格條件相反。

  vol_ratio = 當日成交量 / MA20
  MA20      = 前 20 個交易日的均量,shift(1) — 不含當日
  暴跌條件  = vol_ratio > 1.5 且 close < open

輸出 data/crash-latest.json(前端讀這支)與 data/crashes/YYYY-MM-DD.json(存查)。

用法:
  python scripts/compute_crash.py
      跑最新一天(daily-scan.yml 用這個,讀 data/history)
  python scripts/compute_crash.py --backfill-archive [--force]
      一次性:用 data/archive/prices(現有存檔,不打 API)回補所有歷史日期的
      data/crashes/*.json,公式與每日模式完全一字不改(compute_for_date() 兩邊
...
def run_today():
    ...
    prev = common.read_json(common.CRASH_LATEST_FILE)
    if prev and _without_timestamp(prev) == _without_timestamp(result):
        print("  結果與上次相同(%s),不重寫檔案" % target)
        return 0

    common.write_json(common.CRASH_LATEST_FILE, result)
    common.write_json(
        common.history_path(target).replace(common.HISTORY_DIR, common.CRASHES_DIR),
        result)

    print("  掃描 %d 檔,基線足夠 %d 檔,暴跌 %d 檔" % (result["universe"], result["evaluated"], result["count"]))
...
def run_backfill(force):
    """一次性用 data/archive/prices 回補所有歷史日期的 data/crashes/*.json,
    ...
    for pos in range(common.MA_WINDOW, len(dates)):
        target = dates[pos]
        path = os.path.join(common.CRASHES_DIR, target + ".json")
    ...
    print("== 暴跌掃描回補完成:寫入 %d 天、跳過已存在 %d 天、無資料 %d 天(價格範圍 %s ~ %s) =="
