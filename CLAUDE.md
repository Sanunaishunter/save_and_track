...
- **2026-09-20 量(張)欄標近30個交易日最高/最低**(`computeLookupVolumeExtreme()`):
  ...
  **同一天使用者接著要求把比值直接標在最低那格旁邊**(ex `391.4(1.1%)`),
  只加在最低那個儲存格(最高那格維持純數字,比值本來就是相對最高算的,標在
  最高旁邊沒有意義)、`title` 也同步補上文字版;meta 摘要行沒有拿掉,兩處
  同時存在——摘要行是保底(最低那天常被低活躍度濾掉不出現在表格裡),儲存格
  是「剛好看得到那天」時的就地讀數,不互斥。Playwright 對 1528(2026-08-24
  最低 391.4 張、pct 1.1%)驗證儲存格文字精確等於 `391.4(1.1%)`、`title`
  含 `1.1%`、meta 摘要行沒被動到,無 JS 錯誤。
  **2026-09-20 同一天使用者再追加澄清方向:最低量只看「爆量那天(最高量那天)
  之後」的交易日,爆量之前的低量不算、不標**——蓄勢期的量不是重點,爆量後
  量縮(有沒有人追價、量是不是迅速萎縮)才是該觀察的訊號。用
  `AskUserQuestion` 確認過(原始文字「往前看/往後不用標」跟範例「D-1不標、
  D+1標」字面上互相矛盾,直接猜會有一半機率做反),使用者選的是**只標爆量日
  之後(D+1, D+2…)**。`computeLookupVolumeExtreme()` 改成先找視窗內最高量
  那天的 index(`maxIdx`),`minRow` 只在 `windowRows` 裡 `maxIdx` 之後的區間
  找,不再看整個 30 天視窗;爆量日剛好是視窗最後一天(還沒有爆量後的資料)時
  `minDate`/`minVolume`/`pct` 全部是 `null`,meta 顯示「(爆量後還沒有交易日
  可比)」、表格不標任何 `lookup-vol-min` 儲存格——呼叫端(meta 組字、儲存格
  class 判斷)都要處理這個 null case,`r.date === volExtreme.minDate` 這個
  既有比較式在 `minDate` 是 null 時自然不會命中任何列,不用另外加防呆。
  Playwright 18 項驗證:1528(爆量 09-15 → 之後最低 09-18,21.0%)最高格
  class 正確、最低格文字精確等於 `7,793.2(21.0%)`、**且爆量日之前所有列
  逐一檢查過都沒有 `lookup-vol-min` class**;2313/2345 兩個分頁同樣核對過
  meta 文字+儲存格文字;7610(爆量日剛好是視窗最後一天)驗證 meta 顯示
  「爆量後還沒有交易日可比」、整張表 0 個 `lookup-vol-min` 儲存格、`lookup-
  vol-max` 儲存格還在(=1 個),無 JS 錯誤。
...
- 記分板:六種訊號 × 5/10/20 日,超額 = 個股報酬 − 指數同期,命中 = dir × 超額 > 0;
  ...
  **第一次跑(2026-09-11 資料、30 天)的基準:爆量 5 日 n=362 命中 21.5%、超額 −3.26%;
  ...
  crash 16156 筆),新數字見 commit b25ff69,上面這段(小樣本時期)的結論待重新確認。**
...
- **2026-09-18 記分板加 `bias` 分桶(訊號日 20 日乖離率)**,驗「離均線越遠、反著用是不是
  越強」。乖離 = (收盤 − MA20)/MA20,MA20 含當日(跟 `ma` 分桶同一個 `common.ma_state()`,
  不是 vol_ratio 的 shift 1);五桶有正負號(±5%、±10% 是坊間經驗值,只拿來切樣本不當
  訊號),桶名 `BIAS_BINS` 跟前端 `SC_BUCKET_ORDER` 要一字不差,`scBucketTable()` 有
  order 的分桶照自然順序排、其他維持 n 遞減。結果(本機存檔重跑,`scorecard-latest.json`
  照慣例沒 commit,等排程產):**爆量看多的輸,幾乎集中在正乖離 ≥10% 那桶**——scan 5 日
  ≥+10% n=140 命中 25.0%、超額 −3.71%,10 日 n=82 命中 18.3%、超額 −6.33%;
  −5~+5% 那桶 5 日 n=257 命中 35.0%、超額 −1.37%,10 日 n=150 命中 42.7%、超額 −0.47%
  (接近拋硬幣)。薄股 4000+ 筆同型態:≥+10% 20 日 n=192 命中 18.2%、超額 −8.68%,
  −5~+5% n=3100 命中 20.9%、超額 −5.73%,兩桶都輸但乖離大的輸更多。暴跌(看空)各桶
...
- **`Report/report_20260917.md` 踩過的坑**:虛漲(fomo_fake)`dir=-1`,「最佳出場」
  依 `dir` 鏡射是「期間最低價」,45.9% 超過 10% 代表股價跌超過 10%(確實回檔),
  不是漲超過 10%;原文誤寫成「反著用」跟「繼續噴大」,方向剛好反了,2026-09-18 改正。
- **2026-09-18 `report_20260917.md` 第二三段改成從訊號隔天起算**:期間改成
  `pos+1` 到 `data/archive/prices` 末端(不是固定天數),用回補後的一年份
  樣本重跑(scan 19,198、crash 16,123 筆)。**發現一個更大的問題**:窗口
  長度隨訊號日期系統性變化(越早的訊號能掃的天數越長),這個效應強到
  主導了幾乎所有聚合統計(全部合併中位數從 14 天樣本的個位數飆到
  16.49%、scan 按成交值分桶從單調不明顯變成 Q1 平均 100.26% 的乾淨單調
  遞減、「系統性賣壓」regime 均值暴低是因為那兩天剛好在資料末端),
...
- **2026-09-19 記分板加 Wilson 95% 信賴區間**:`_wilson_ci(hits, n)` 存進
  `hit_ci`/`hit_ci_ew`,不用常態近似(n 小會跑出 0~100 外)。前端只在
  「vs中位股」那組(基準無偏,依定義 50%)區間不含 50% 時標 `sc-sig`
  (粗體+底線);「vs指數」那組(基準有偏)只在 title 附區間,不標粗。
  **順手修的坑**:`scStatCells()` 的 `!st.n` 早退分支只回 3 個 `<td>`,
