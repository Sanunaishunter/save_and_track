...
### 量縮轉買標記(2026-09-08 加入,純前端)

`js/app.js` 的 `computeLookupBreakouts()`,吃這台表格既有的
volume/foreign_net/trust_net,不用新資料源、不改 `stock-lookup-latest.json`
的結構。討論脈絡:2313 在 8/12~8/25 量縮、8/26 外資投信同步轉買、量能
放大,使用者問「程式有辦法自動抓這種模式嗎」而加的。

條件(對真實 2313 資料反覆調過,不是憑空定的):
...
**沒有加「隔天不能反轉」的過濾。** 8/20 那天其實也符合以上三條(外資
