...
跟 🔔量縮轉買同一套窗口/門檻,差別是不要求「今天」有外資投信同步買超+
量增——只看「最新一天之前的 10 個交易日裡,量縮天數夠不夠」。使用者要
的其實是這個:不是回頭看哪天觸發過,是現在就抓「已經蹲夠天數,只差
題材/轉買訊號」的候選,當進場前的觀察名單。個股查詢頁面選到符合條件
的股票時,「資料範圍」那行會多一句 🕐 提示;`probe_signal_batch.py`
批次測試會把這類候選另外列成第四欄。
