...
使用者另外指定要記錄(還沒決定要不要用,先存起來):

- **`opendata/t187ap18` 證券商基本資料** —— 標題完全對上「券商基本資料」。
  另外 `brokerService/brokerList`(證券商總公司基本資料)名字很像但看起來是
  不同資料集,兩支都存在,實際差異還沒探測過
- **`ETFReport/ETFRank` 定期定額交易戶數統計排行月報表** —— 唯一符合
  「定期定額統計」的端點,是月報表(不是逐日),`brokerService/secRegData`
  (開辦定期定額業務證券商名單)是相關但不同的資料(名單,不是統計)

以上全部只確認端點存在、HTTP 200、swagger 有登記,**沒有探測過實際回應的
欄位長什麼樣**——照專案慣例,真的要用之前還是要先寫 probe 腳本印出真實
回應,不能照 summary 文字猜欄位。
