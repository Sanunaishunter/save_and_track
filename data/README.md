...
### 量縮 / 量縮轉買標記(2026-09-08 加入,純前端)

`js/app.js` 的 `computeLookupShrinkDays()` + `computeLookupBreakouts()`,
兩個都吃這台表格既有的 volume/foreign_net/trust_net,不用新資料源、不改
`stock-lookup-latest.json` 的結構,共用同一份 `lookupVolRatios()` 算出來的
「量 / 近期高點」比值。討論脈絡:2313 在 8/12~8/25 量縮、8/26 外資投信
同步轉買、量能放大,使用者問「程式有辦法自動抓這種模式嗎」而加的。
