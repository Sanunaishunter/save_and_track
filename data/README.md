...
`computeLookupSelloffDays()`,三個都吃這台表格既有的
volume/foreign_net/trust_net/open/close,不用新資料源、不改
`stock-lookup-latest.json` 的結構;前兩個共用同一份 `lookupVolRatios()`
算出來的「量 / 近期高點」比值。討論脈絡:2313 在 8/12~8/25 量縮、8/26
外資投信同步轉買、量能放大,使用者問「程式有辦法自動抓這種模式嗎」而加的;
外資出貨是同一輪討論延伸,對照 7/30 大跌那天。

**🔽量縮(單日):** 當天「量 / 近期高點(前 10 天內最高量)」< 0.3 就標,
不管有沒有接著轉買。使用者要的不只是「轉買那天才看得到」,量縮期間本身
也要能一眼看出來,自己判斷什麼時候要進場。
...
的動機)直接消失——8/26 前 5 天只有 2 天壓得到 0.4 以下,🔔 要求的
「3/5 天量縮」門檻就不成立。所以拆成兩個獨立常數:🔔轉買邏輯裡用來算
「這 5 天裡有幾天量縮」的門檻維持 0.6 不動,只有 🔽 這個純顯示用的單日
標記另外收緊到 0.3(批次測試命中率降到 27%)。改前端記得同時改
`js/app.js` 的 `LOOKUP_BREAKOUT_SHRINK_RATIO`(給 🔔,別動)跟
`LOOKUP_SHRINK_DISPLAY_RATIO`(給 🔽)兩個常數,以及 `probe_signal_batch.py`
對應的 `SHRINK_RATIO`/`SHRINK_DISPLAY_RATIO`,兩邊定義要一致。

**🔔量縮轉買(組合訊號):**

1. 觸發日前 5 個交易日裡,至少 3 天「量 / 近期高點(前 10 天內最高量)」
   < 0.6
2. 觸發日當天外資、投信買賣超同步 > 0
...
**沒有加「隔天不能反轉」的過濾。** 8/20 那天其實也符合🔔的三條件(外資
投信同步買超、量能放大 1.91 倍),但隔天(8/21)外資馬上翻臉倒貨、股價
回落。規則不假裝能預測後續走勢,兩天都標——使用者測過這個取捨,要求
保留 8/20 一起標,不加「觸發後要撐住」的過濾條件。
