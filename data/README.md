...
以下是看過全部 143 個之後,挑出來覺得跟這個專案方向有關、值得記住的:

- **`exchangeReport/TWT84U` 上市個股股價升降幅度** —— 官方漲跌停旗標。
  現在爆量掃描判斷漲停是用 `high == close` 推測,這支能拿到正式資料,不用猜
- **`announcement/notice` 集中市場當日公布注意股票** —— 爆量股常被標注意股,
  可以疊加在爆量掃描/FOMO 當風險提示
- **`exchangeReport/MI_MARGN` 集中市場融資融券餘額** —— 全市場一次拿到。
  FOMO 目前融資融券資料來自 FinMind(60 檔額度上限的瓶頸之一),如果欄位
  對得上,換這支可以省下一部分 FinMind 額度
- **`exchangeReport/BFI84U` 集中市場停資停券預告表** —— 買進前先看會不會
  隔天就停資停券
- **`exchangeReport/TWT48U_ALL` 上市股票除權除息預告表** —— 排除除權息
  造成的假爆量
- **`exchangeReport/FMTQIK` 集中市場每日市場成交資訊** —— 大盤當日總成交量,
  可算「這檔佔大盤成交比重」
