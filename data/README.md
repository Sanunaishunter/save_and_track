...
| `thin-scans/YYYY-MM-DD.json` | 當日薄股測試結果存查(跟爆量掃描同公式,鎖定量 < 300 張這群,見下「薄股測試」一節) |
| `thin-scan-latest.json` | 最新一次薄股測試結果,前端讀這支 |
...
起因:使用者拿 6957 裕慶-KY 的真實線圖追出來,這檔股票的量在噴出前
(9/10、9/11,甚至更早的 2025-08-19、08-28)vol_ratio 其實早就超過爆量
掃描的 1.5 門檻,只是每次量都不到 300 張(`common.MIN_VOLUME_LOTS`),被
現行爆量掃描的絕對量下限整批濾掉,一路到量衝上 390 張(9/16)才第一次
進爆量掃描名單。

`compute_thin_scan.py`:跟爆量掃描**公式一字不改**(`vol_ratio > 1.5` 且
`close > open`),差別只在鎖定「量 < 300 張」這群——共用同一份
`data/history`,兩個掃描完全互補、同一天同一檔不會同時出現在兩份清單裡。
下限 `THIN_SCAN_MIN_VOLUME_LOTS`(10 張)只是排除接近零成交的雜訊,不是
篩「好」股票。

用全市場資料(`data/archive/prices`,284 天、不打 API)回測過,結論寫在
`scripts/compute_thin_scan.py` 開頭跟 `CLAUDE.md` 第 5 節:量 < 300 張這群
平均命中率比爆量掃描本身(量 >= 300 張)更低、超額報酬更負,兩群都是
「看多輸」,薄股這群輸得更多——不是新發現的獨立優勢,是已經驗證過的
「爆量看多容易輸」故事的加強版。

使用者的決定:不是要拿這個當保證勝率的進場訊號,是「跟著散戶一起抬轎、
有紀律地下車」——照樣做成正式訊號、進記分板(`compute_scorecard.py` 的
`thin_scan`,`SIGNAL_DEFS` 跟其他訊號同一套 `dir`/`label` 格式)算勝率,
讓使用者自己盯著數字判斷要不要跟、跟多重,不是系統幫他下結論說這穩賺。
因為已經驗證過(不是沒驗證過的教科書公式),前端不用 📖 標記。

一次性回補:`data/thin-scans/*.json` 是用 `compute_thin_scan.py
--backfill-archive` 從既有的 `data/archive/prices`(2025-07 起)一次算出來
的,不是從上線那天才開始累積(這點跟 scan/crash/FOMO 幾個訊號不同,那些
沒有回補歷史,是每天排程自然長出樣本)——因為原始價量資料本來就已經在
archive 裡,回補純粹是重新計算,不是補抓新資料,做一次就能讓記分板立刻
有 4000+ 筆到期樣本可以看,不用再等好幾週。之後每天 `daily-scan.yml` 照樣
正常累加新的一天。
...
(1.5~2 / 2~3 / 3~5 / 5+)、`bias`(2026-09-18 加:訊號日 20 日乖離率
